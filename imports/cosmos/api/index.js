...
// TODO: We are going to make this more modular, build based on configs
const DEFAULTS = {};
DEFAULTS.packages = []; // potentially staking, minting etc.
DEFAULTS.RESTurl = "http://127.0.0.1:1317";
...
export default class CosmosAPI {
  constructor (config) {
    ...
    this.chainId = config && config.chainId || DEFAULTS.chainId
    this.get = {}
    ...
    const getters = _Getters(this.url)
    Object.keys(getters).forEach(getter => {
      this.get[getter] = getters[getter]
    })

    // add message constructors to the Sender to provide a simple API for transactions
